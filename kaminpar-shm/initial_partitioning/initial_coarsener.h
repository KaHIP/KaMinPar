...
struct InitialCoarseningCluster {
...
struct InitialCoarseningMemoryContext {
  std::vector<InitialCoarseningCluster> clustering{};
  ...
  [[nodiscard]] std::size_t memory_in_kb() const {
    return clustering.size() * sizeof(InitialCoarseningCluster) / 1000 + //
...
public:
  ...
  InitialCoarsener(
      const Graph *graph,
      const InitialCoarseningContext &c_ctx,
      InitialCoarseningMemoryContext &&m_ctx
  )
      : _input_graph(graph),
  ...
  InitialCoarsener(const Graph *graph, const InitialCoarseningContext &c_ctx)
      : InitialCoarsener(graph, c_ctx, InitialCoarseningMemoryContext{}) {}
  ...
  InitialCoarseningMemoryContext free() {
    return {
  ...
  NodeID
  pick_cluster_from_rating_map(NodeID u, NodeWeight u_weight, NodeWeight max_cluster_weight) {
...
private:
  ...
  SequentialGraphHierarchy<Graph> _hierarchy;
