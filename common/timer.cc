...
void Timer::print_children_hr(
  ...
  for (const auto &child : node->children) {
    ...
    if (!child->annotation.empty()) {
      out << std::string(kSpaceBetweenRestartsAndAnnotation, ' ')
          << child->annotation;
    }
    out << std::endl;
...
void Timer::print_padded_timing(
  ...
  // print this node
  const std::size_t time_padding_len =
      _hr_time_col - start_col - kNameDel.size();
  std::string time_padding =
      time_padding_len > 0 ? std::string(time_padding_len - 1, kPadding) + ' '
                           : "";
  const double time = node->seconds();
  out << kNameDel << time_padding << std::fixed << std::setprecision(3) << time
      << kSecondsUnit;
  ...
  const std::size_t tail_padding_length =
      _hr_max_restarts_len - get_printed_length(node->restarts);
...
[[nodiscard]] std::size_t Timer::compute_time_col(
  ...
  std::size_t col =
      prefix_len + node->build_display_name_hr().size() + kNameDel.size();
  for (const auto &child : node->children) {
