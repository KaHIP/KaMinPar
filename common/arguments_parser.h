...
public:
    ...
    static std::string parse_string(const char* arg) {
        return arg;
    }
    ...
    struct Group {
        ...
        Group&
        argument(const std::string& lname, const std::string& description, Float* storage, const char sname = 0) {
        ...
        template <typename Type>
        Group&
        opt_argument(const std::string& lname, const std::string& description, Type* storage, const char sname = 0) {
        ...
        Group&
        argument(const std::string& lname, const std::string& description, std::string* storage, const char sname = 0) {
            return argument(lname, description, storage, Arguments::parse_string, sname);
        }

        //! Argument whose value is a bool. `1`, `true`, `on`, `yes` and <empty> are interpreted as `true`, all other
        //! values as `false`.
        ...
        /**
         * Adds an argument with custom data type. Must supply an invocable object that transforms the value passed to
         * the argument to the custom data type.
         *
    ...
    void parse(int argc, char* argv[], const bool enforce_positional_arguments = true) {
        ...
        if (!positional_group.arguments.empty()) {
            ...
            const bool        has_vararg =
                std::any_of(positional_group.arguments.begin(), positional_group.arguments.end(), [](const auto& arg) {
                    return arg.vararg;
                });
...
private:
    ...
    [[nodiscard]] std::vector<Argument>::const_iterator find_by_short_name(const char short_name) const {
        return std::find_if(arguments.begin(), arguments.end(), [&short_name](const Argument& arg) {
            return arg.short_name == short_name;
        });
    }

    [[nodiscard]] std::vector<Argument>::const_iterator find_by_long_name(const std::string& long_name) const {
        return std::find_if(arguments.begin(), arguments.end(), [&long_name](const Argument& arg) {
