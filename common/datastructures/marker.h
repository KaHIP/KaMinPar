...
namespace kaminpar {
template <std::size_t num_concurrent_markers = 1,
          typename element_type = std::size_t>
class Marker {
public:
  explicit Marker(const std::size_t capacity)
      : _data(capacity), _marker_id(0), _first_unmarked_element{0} {}
  ...
  void set(const std::size_t element, const std::size_t marker = 0) {
    ...
    _data[element] = ((_data[element] &
                       ~((1u << num_concurrent_markers) - 1u)) == _marker_id)
  ...
  [[nodiscard]] bool get(const std::size_t element,
                         const std::size_t marker = 0) const {
    KASSERT(marker < num_concurrent_markers);
