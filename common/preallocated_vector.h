...
public:
    ...
    PreallocatedAllocator(T* storage, const size_type size) throw() : _storage(storage), _size(size) {}

    PreallocatedAllocator(const PreallocatedAllocator& rhs) throw() : _storage(rhs._storage), _size(rhs._size) {}
    ...
    pointer allocate(const size_type n) {
        KASSERT(n == _size, "allocation request does not match the preallocated storage", assert::light);
        _size = 0;
        return _storage;
    }
...
template <typename T>
PreallocatedVector<T> make_preallocated_vector(T* storage, const std::size_t start, const std::size_t size) {
...
template <typename Container>
auto make_preallocated_vector(Container& storage) -> PreallocatedVector<typename Container::value_type> {
