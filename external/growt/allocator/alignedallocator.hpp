...
template <class T = char, size_t A = DEFAULT_ALIGNMENT> class GenericAlignedAllocator {
public:
  using value_type = T;
  using pointer = T *;
  ...
  template <class U> GenericAlignedAllocator(const GenericAlignedAllocator<U> &) noexcept {}
  ...
  pointer allocate(size_type n, const void * /* hint */ = nullptr) {
    ...
    if (n > max_size())
      ...
      // @todo(heap-profiler): the "else" branch should also work for linux, but the heap profiler
      // only tracks allocations through memalign
#ifdef __linux__
  ...
  //! Constructs an element object on the location pointed by p.
  template <typename SubType, typename... Args> void construct(SubType *p, Args &&...args) {
...
template <typename E = char> using AlignedAllocator = GenericAlignedAllocator<E, DEFAULT_ALIGNMENT>;
