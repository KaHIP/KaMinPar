...
NodePermutations<Container> sort_by_degree_buckets(const Container<EdgeID> &nodes) {
  auto find_bucket = [&](const Degree deg) { return deg == 0 ? kNumberOfDegreeBuckets - 1 : degree_bucket(deg); };
...
void build_permuted_graph(const Container<EdgeID> &old_nodes, const Container<NodeID> &old_edges,
                          ...
                          Container<NodeID> &new_edges, Container<NodeWeight> &new_node_weights,
                          Container<EdgeWeight> &new_edge_weights) {
