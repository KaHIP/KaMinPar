...
PartitionedGraph ParallelSynchronizedInitialPartitioner::partition(
  ...
  while (num_current_copies < num_threads) {
    ...
    tbb::parallel_for(static_cast<std::size_t>(0), num_current_copies,
                      [&](const std::size_t i) {
                        ...
                        for (std::size_t next_j = next_i;
                             next_j < next_i + num_local_copies; ++next_j) {
    ...
    tbb::parallel_for(static_cast<std::size_t>(0), num_current_copies,
                      [&](const std::size_t i) {
                        const bool shrunk = helper::coarsen_once(
                            ...
                            next_coarseners[i]->coarsest_graph(), _input_ctx,
                            current_p_ctxs[i]);
                        if (shrunk) {
                          converged = false;
                        }
  ...
  tbb::parallel_for(
      static_cast<std::size_t>(0), num_threads, [&](const std::size_t i) {
        auto &current_coarseners = coarseners.back();
        const Graph *graph = current_coarseners[i]->coarsest_graph();
        current_p_graphs[i] = helper::bipartition(graph, _input_ctx.partition.k,
  ...
  KASSERT(&(current_p_graphs.front().graph()) == coarsener->coarsest_graph(),
          "", assert::light);
