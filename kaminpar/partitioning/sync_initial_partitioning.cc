...
PartitionedGraph SyncInitialPartitioner::partition(
  ...
  while (num_current_copies < num_threads) {
    ...
    const std::size_t num_local_copies = helper::compute_num_copies(
        _input_ctx, n, converged, num_current_threads
    );
    ...
    std::vector<PartitionContext> next_p_ctxs(
    ...
    tbb::parallel_for(
        ...
        [&](const std::size_t i) {
          ...
          for (std::size_t next_j = next_i; next_j < next_i + num_local_copies;
               ++next_j) {
            DBG << "Duplicate " << i << " --> " << next_j;
            next_coarseners[next_j] =
                duplicate_coarsener(current_coarseners[i].get());
  ...
  KASSERT(
      &(current_p_graphs.front().graph()) == coarsener->coarsest_graph(),
      "",
      assert::light
  );
...
SyncInitialPartitioner::duplicate_coarsener(const Coarsener *coarsener) {
  return factory::create_coarsener(
      *coarsener->coarsest_graph(), _input_ctx.coarsening
  );
}
} // namespace kaminpar::shm::partitioning
