...
struct InitialPartitionerMemoryPool {
  ...
  void put(ip::InitialPartitioner::MemoryContext m_ctx) {
...
void extend_partition_recursive(
    ...
    BlockID final_k, const Context &input_ctx,
...
void extend_partition(PartitionedGraph &p_graph, BlockID k_prime,
...
std::size_t compute_num_copies(const Context &input_ctx, NodeID n,
                               bool converged, std::size_t num_threads);

std::size_t select_best(const scalable_vector<PartitionedGraph> &p_graphs,
                        const PartitionContext &p_ctx);

template <typename Iterator>
std::size_t select_best(const Iterator p_graphs_begin,
                        const Iterator p_graphs_end,
                        const PartitionContext &p_ctx) {
  SET_DEBUG(false);

  KASSERT(p_graphs_begin < p_graphs_end,
