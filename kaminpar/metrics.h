...
EdgeWeight edge_cut(const PartitionedGraphType &p_graph, tag::Parallel) {
  ...
  tbb::parallel_for(
      tbb::blocked_range(static_cast<NodeID>(0), p_graph.n()),
      [&](const auto &r) {
        auto &cut = cut_ets.local();
        for (NodeID u = r.begin(); u < r.end(); ++u) {
          for (const auto &[e, v] : p_graph.neighbors(u)) {
            cut += (p_graph.block(u) != p_graph.block(v))
                       ? p_graph.edge_weight(e)
                       : 0;
          }
        }
      }
...
EdgeWeight edge_cut(const PartitionedGraphType &p_graph, tag::Sequential) {
  ...
  for (const NodeID u : p_graph.nodes()) {
    for (const auto &[e, v] : p_graph.neighbors(u)) {
      cut +=
          (p_graph.block(u) != p_graph.block(v)) ? p_graph.edge_weight(e) : 0;
    }
  }
...
template <typename PartitionedGraphType>
EdgeWeight edge_cut(const PartitionedGraphType &p_graph) {
  return edge_cut(p_graph, tag::par);
}

template <typename PartitionedGraphType>
double imbalance(const PartitionedGraphType &p_graph) {
  const NodeWeight total_weight = p_graph.total_node_weight();
  const double perfect_block_weight =
      std::ceil(static_cast<double>(total_weight) / p_graph.k());

  double max_imbalance = 0.0;
  for (const BlockID b : p_graph.blocks()) {
    max_imbalance = std::max(
        max_imbalance, p_graph.block_weight(b) / perfect_block_weight - 1.0
    );
  }
...
NodeWeight total_overload(
    const PartitionedGraphType &p_graph, const PartitionContext &context
) {
  NodeWeight total_overload = 0;
  for (const BlockID b : p_graph.blocks()) {
    total_overload += std::max<BlockWeight>(
        0, p_graph.block_weight(b) - context.block_weights.max(b)
    );
  }
...
template <typename PartitionedGraphType>
bool is_balanced(
    const PartitionedGraphType &p_graph, const PartitionContext &p_ctx
) {
  return std::all_of(
...
template <typename PartitionedGraphType>
bool is_feasible(
    const PartitionedGraphType &p_graph, const BlockID input_k, const double eps
) {
  const double max_block_weight =
      std::ceil((1.0 + eps) * p_graph.total_node_weight() / input_k);
  return std::all_of(
      p_graph.blocks().begin(),
      p_graph.blocks().end(),
      [&p_graph, max_block_weight](const BlockID b) {
        return p_graph.block_weight(b) <=
               max_block_weight * p_graph.final_k(b) +
                   p_graph.max_node_weight();
      }
  );
}
