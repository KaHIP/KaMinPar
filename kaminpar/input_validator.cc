...
void validate_undirected_graph(
    const StaticArray<EdgeID>& nodes, const StaticArray<NodeID>& edges, const StaticArray<NodeWeight>&,
    ...
    tbb::parallel_for<NodeID>(0, n, [&](const NodeID u) {
        std::sort(
            edges_with_weights.begin() + nodes[u], edges_with_weights.begin() + nodes[u + 1],
        ...
        for (EdgeID e = nodes[u] + 1; e < nodes[u + 1]; ++e) {
            if (edges[e - 1] == edges[e]) {
                LOG_ERROR << "node " << u + 1 << " has multiple edges to neighbor " << edges[e];
                std::exit(1);
            }
        }
    });
    ...
    tbb::parallel_for<NodeID>(0, n, [&](const NodeID u) {
        for (EdgeID e = nodes[u]; e < nodes[u + 1]; ++e) {
            ...
            if (rev == it_end) {
                LOG_ERROR << "missing reverse edge: of edge " << u + 1 << " --> " << v + 1
