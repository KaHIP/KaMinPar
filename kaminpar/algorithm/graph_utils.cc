...
void fill_final_k(scalable_vector<BlockID> &data, const BlockID b0, const BlockID final_k, const BlockID k) {
  ASSERT(final_k >= k) << V(final_k) << V(k);
  
  const auto [final_k1, final_k2] = math::split_integral(final_k);
  ASSERT(final_k1 > 0) << V(final_k1) << V(final_k2) << V(final_k);
  ASSERT(final_k2 > 0) << V(final_k1) << V(final_k2) << V(final_k);
...
void copy_subgraph_partitions(PartitionedGraph &p_graph,
  ...
  if (k_prime != input_k) {
    ASSERT(math::is_power_of_2(k_prime));
  ...
  ASSERT(std::accumulate(p_graph.final_ks().begin(), p_graph.final_ks().end(), 0) == std::accumulate(final_ks.begin(), final_ks.end(), 0))
  ...
  tbb::parallel_for(static_cast<NodeID>(0), p_graph.n(), [&](const NodeID &u) {
    ...
    ASSERT(k0[b] + p_subgraph_partitions[b][s_u] < k_prime) << V(k_prime) << V(k0[b]) << V(p_subgraph_partitions[b][s_u]) << V(b) << V(s_u);
