...
public:
  ...
  void set(const std::size_t element, const std::size_t marker = 0) {
    ...
    if constexpr (track_first_unmarked_element) {
      while (_first_unmarked_element[marker] < _data.size() && get(_first_unmarked_element[marker], marker)) {
  ...
  [[nodiscard]] inline std::size_t size() const { return _data.size(); }
  [[nodiscard]] inline std::size_t capacity() const { return size(); }

  void reset() { // increase such that the least significant num_concurrent_markers bits are zeroed
    _marker_id |= (1u << num_concurrent_markers) - 1u;
    _marker_id += 1u;
    _first_unmarked_element.fill(0);

    if ((_marker_id | ((1u << num_concurrent_markers) - 1u)) == std::numeric_limits<element_type>::max()) {
      _marker_id = 0;
