...
struct PartitionContext {
  ...
  NodeWeight total_node_weight =
      kInvalidNodeWeight; //! Total node weight in the input graph.
  EdgeWeight total_edge_weight =
      kInvalidEdgeWeight; //! Total edge weight in the input graph.
  NodeWeight max_node_weight =
      kInvalidNodeWeight; //! Weight of heaviest node in the input graph.
...
struct LabelPropagationCoarseningContext {
  ...
  double two_hop_clustering_threshold; //! Perform 2-hop clustering if graph
                                       //! shrunk by less than this factor.
...
struct CoarseningContext {
  ...
  LabelPropagationCoarseningContext
      lp;                   //! Configuration for LP as clustering algorithm.
  NodeID contraction_limit; //! Abort coarsening if the coarsest graph has less
                            //! than twice this many nodes.
  bool enforce_contraction_limit; //! Force the clustering algorithm to converge
                                  //! on the contraction limit.
  double convergence_threshold; //! Coarsening converges if the graph shrunk by
                                //! less than this factor.
  ClusterWeightLimit
      cluster_weight_limit; //! Rule to compute the maximum cluster weight.
  double cluster_weight_multiplier; //! Multiplicative factor to the maximum
                                    //! cluster weight.
...
struct FMRefinementContext {
  ...
  NodeID num_fruitless_moves;   //! Stop after more than this many moves without
                                //! cut improvement.
  double alpha; //! Config parameter of the adaptive stopping criteria.
  std::size_t num_iterations;            //! Maximum number of FM iterations.
  double improvement_abortion_threshold; //! Stop FM if last iteration improved
                                         //! the cut by less than this factor.
...
struct BalancerRefinementContext {
  ...
  BalancingTimepoint
      timepoint; //! Rule to determine when to run the balancing algorithm.
...
struct RefinementContext {
  ...
  BalancerRefinementContext
      balancer; //! If a balancer is used: configuration for balancer.
