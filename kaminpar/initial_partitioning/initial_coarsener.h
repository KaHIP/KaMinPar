...
public:
  ...
  const Graph *
  coarsen(const std::function<NodeWeight(NodeID)> &cb_max_cluster_weight);
  ...
  void TEST_mock_clustering(const std::vector<NodeID> &clustering) {
    ...
    for (const NodeID u : _current_graph->nodes()) {
      ...
      _clustering[_clustering[u].leader].weight +=
          _current_graph->node_weight(u);
      if (u != clustering[u]) {
