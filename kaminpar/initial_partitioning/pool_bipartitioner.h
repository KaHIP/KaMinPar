...
public:
  struct MemoryContext {
    ...
    std::size_t memory_in_kb() const {
      return ggg_m_ctx.memory_in_kb() + bfs_m_ctx.memory_in_kb() + rand_m_ctx.memory_in_kb() + ref_m_ctx.memory_in_kb();
    }
  };
  ...
  const std::string &bipartitioner_name(const std::size_t i) { return _bipartitioner_names[i]; }
  ...
  PartitionedGraph bipartition() {
    ...
    // only perform more repetitions with bipartitioners that are somewhat likely to find a better partition than the
    // current one
    const auto repetitions = std::clamp(_num_repetitions, _min_num_repetitions, _max_num_repetitions);
    for (std::size_t rep = 0; rep < repetitions; ++rep) {
      for (std::size_t i = 0; i < _bipartitioners.size(); ++i) {
        if (rep < _min_num_non_adaptive_repetitions || !_i_ctx.use_adaptive_bipartitioner_selection ||
            likely_to_improve(i)) {
          run_bipartitioner(i);
        }
      }
  ...
  void set_num_repetitions(const std::size_t num_repetitions) { _num_repetitions = num_repetitions; }
...
private:
  ...
  void print_statistics() {
    ...
    for (std::size_t i = 0; i < _bipartitioners.size(); ++i) {
      ...
      LOG << logger::CYAN << "  * cut_mean=" << stats.cut_mean << " cut_variance=" << stats.cut_variance
          << " cut_std_dev=" << std::sqrt(stats.cut_variance);
    }

    LOG << logger::CYAN << "Winner: " << _bipartitioner_names[_best_bipartitioner];
    LOG << logger::CYAN << " * cut=" << _best_cut << " imbalance=" << _best_imbalance << " feasible=" << _best_feasible;
    LOG << logger::CYAN << "# of runs: " << num_runs_total << " of "
        << _bipartitioners.size() * std::clamp(_num_repetitions, _min_num_repetitions, _max_num_repetitions);
  }
  ...
  void run_bipartitioner(const std::size_t i) {
    ...
    if (_best_feasible <= current_feasible && (_best_feasible < current_feasible || current_cut < _best_cut ||
...
public:
  std::unique_ptr<PoolBipartitioner> create(const Graph &graph, const PartitionContext &p_ctx,
    ...
    pool->register_bipartitioner<LighterBlockBfsBipartitioner>("bfs_lighter_block", pool->_m_ctx.bfs_m_ctx);
    pool->register_bipartitioner<LongerQueueBfsBipartitioner>("bfs_longer_queue", pool->_m_ctx.bfs_m_ctx);
    pool->register_bipartitioner<ShorterQueueBfsBipartitioner>("bfs_shorter_queue", pool->_m_ctx.bfs_m_ctx);
    pool->register_bipartitioner<SequentialBfsBipartitioner>("bfs_sequential", pool->_m_ctx.bfs_m_ctx);
