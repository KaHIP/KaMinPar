...
struct alternating {
  BlockID operator()(const BlockID active_block, const Bipartitioner::BlockWeights &, const PartitionContext &,
                     const Queues &) {
...
struct lighter {
  BlockID operator()(const BlockID, const Bipartitioner::BlockWeights &block_weights, const PartitionContext &,
                     const Queues &) {
...
struct longer_queue {
  BlockID operator()(const BlockID, const Bipartitioner::BlockWeights &, const PartitionContext &,
                     const Queues &queues) {
...
public:
  ...
  BfsBipartitionerBase(const Graph &graph, const PartitionContext &p_ctx, const InitialPartitioningContext &i_ctx)
      : Bipartitioner(graph, p_ctx, i_ctx) {}
};

/*!
 * Grows two blocks starting from pseudo peripheral nodes using a breath first search.
 *
 * In each step, a node is assigned to one of the two blocks. A block selection strategy is used to decide when to
 * switch between blocks.
 *
 * @tparam block_selection_strategy Invoked after each step to select the active block in the next step.
  ...
  void bipartition_impl() override {
    ...
    while (_marker.first_unmarked_element(kMarkAssigned) < _graph.n()) {
      if (__builtin_expect(_queues[active].empty(), 0)) {
        const auto first_unassigned_node = static_cast<NodeID>(_marker.first_unmarked_element(kMarkAssigned));
      ...
      // node could have been assigned between the time it was put into the queue and now, hence we must check this here
      if (__builtin_expect(!_marker.get(u, kMarkAssigned), 1)) {
        // if the balance constraint does not allow us to put this node into the active block, we switch to the lighter
        // block instead
        // --------------------------------------------------
