...
struct longer_queue {
    BlockID
    operator()(const BlockID, const Bipartitioner::BlockWeights&, const PartitionContext&, const Queues& queues) {
...
protected:
    void bipartition_impl() override {
        ...
        while (_marker.first_unmarked_element(kMarkAssigned) < _graph.n()) {
            ...
            // node could have been assigned between the time it was put into the queue and now, hence we must check
            // this here
            if (__builtin_expect(!_marker.get(u, kMarkAssigned), 1)) {
                // if the balance constraint does not allow us to put this node into the active block, we switch to the
                // lighter block instead
                // --------------------------------------------------
