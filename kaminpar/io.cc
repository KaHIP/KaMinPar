...
namespace metis {
void write_file(std::ofstream &out, const StaticArray<EdgeID> &nodes,
  ...
  if (write_node_weights || write_edge_weights) {
    out << " " << static_cast<int>(write_node_weights)
        << static_cast<int>(write_edge_weights);
  }
  out << "\n";
...
void write_file(const std::string &filename, const StaticArray<EdgeID> &nodes,
                const StaticArray<NodeID> &edges,
                const StaticArray<NodeWeight> &node_weights,
                const StaticArray<EdgeWeight> &edge_weights,
                const std::string &comment) {
...
Statistics read(const std::string &filename, StaticArray<EdgeID> &nodes,
  ...
  parse<false>(
      ...
      [&](const std::uint64_t weight) {
        if constexpr (checked) {
          if (weight > static_cast<std::uint64_t>(
          ...
          KASSERT(weight <= static_cast<std::uint64_t>(
                                std::numeric_limits<NodeWeight>::max()),
      ...
      [&](const std::uint64_t weight, const std::uint64_t v) {
        if constexpr (checked) {
          ...
          if (v + 1 >= nodes.size()) {
            LOG_ERROR << "neighbor " << v + 1 << " of nodes " << u + 1
                      << " is out of bounds";
            std::exit(1);
          }
          if (v + 1 == u) {
            LOG_ERROR << "detected self-loop on node " << v + 1
  ...
  if constexpr (checked) {
    if (stats.total_node_weight >
        static_cast<std::uint64_t>(std::numeric_limits<NodeWeight>::max())) {
      LOG_ERROR << "total node weight does not fit into the node weight type";
      std::exit(1);
    }
    if (stats.total_edge_weight >
...
template Statistics read<false>(const std::string &filename,
                                StaticArray<EdgeID> &nodes,
                                StaticArray<NodeID> &edges,
                                StaticArray<NodeWeight> &node_weights,
                                StaticArray<EdgeWeight> &edge_weights);

template Statistics read<true>(const std::string &filename,
                               StaticArray<EdgeID> &nodes,
                               StaticArray<NodeID> &edges,
                               StaticArray<NodeWeight> &node_weights,
                               StaticArray<EdgeWeight> &edge_weights);
...
template <bool checked>
Graph read(const std::string &filename, bool ignore_node_weights,
           bool ignore_edge_weights) {
  StaticArray<EdgeID> nodes;
  StaticArray<NodeID> edges;
  StaticArray<NodeWeight> node_weights;
  StaticArray<EdgeWeight> edge_weights;
...
template Graph read<false>(const std::string &filename,
                           bool ignore_node_weights, bool ignore_edge_weights);

template Graph read<true>(const std::string &filename, bool ignore_node_weights,
                          bool ignore_edge_weights);

void write(const std::string &filename, const Graph &graph,
           const std::string &comment) {
  metis::write_file(filename, graph.raw_nodes(), graph.raw_edges(),
                    graph.raw_node_weights(), graph.raw_edge_weights(),
                    comment);
}
} // namespace metis
