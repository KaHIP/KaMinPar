      ...
      static_cast<BlockID>(0), _p_graph->k(), [&](const BlockID from) {
        ...
        if (current_overload > 0 && _feasible_target_blocks.local().empty()) {
          ...
          DBG << "Block " << from << " with overload: " << current_overload
              << ": " << _feasible_target_blocks.local().size()
              << " feasible target blocks and " << _pq.size(from)
        ...
        while (current_overload > 0 && !_pq.empty(from)) {
          ...
          if (expected_relative_gain ==
            ...
            if (to ==
                from) { // internal node --> move to random underloaded block
...
bool GreedyBalancer::add_to_pq(const BlockID b, const NodeID u,
  ...
  if (_pq_weight[b] < block_overload(b) || _pq.empty(b) ||
    ...
    DBG << "Add " << u << " pq weight " << _pq_weight[b] << " rel_gain "
        << rel_gain;
...
void GreedyBalancer::init_pq() {
  ...
  tbb::parallel_for(static_cast<BlockID>(0), k, [&](const BlockID b) {
    ...
    if (!_pq.empty(b)) {
      DBG << "PQ " << b << ": weight=" << _pq_weight[b] << ", "
          << _pq.peek_min_key(b) << " < key < " << _pq.peek_max_key(b);
    } else {
      DBG << "PQ " << b << ": empty";
    }
  });
  ...
  auto action = [&](auto &map) {
    ...
    for (const auto [e, v] : _p_graph->neighbors(u)) {
      ...
      if (u_block != v_block && _p_graph->block_weight(v_block) + u_weight <=
                                    _p_ctx->block_weights.max(v_block)) {
...
void GreedyBalancer::init_feasible_target_blocks() {
  ...
  for (const BlockID b : _p_graph->blocks()) {
    if (_p_graph->block_weight(b) <
        _p_ctx->block_weights.perfectly_balanced(b)) {
      blocks.push_back(b);
    }
  }
}
