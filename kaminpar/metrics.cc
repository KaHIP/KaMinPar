...
EdgeWeight edge_cut(const PartitionedGraph &p_graph, tag::Parallel) {
  ...
  tbb::parallel_for(tbb::blocked_range(static_cast<NodeID>(0), p_graph.n()),
                    [&](const auto &r) {
  ...
  KASSERT(
      ...
      "edge cut overflows: " << global_cut, assert::always);
...
EdgeWeight edge_cut(const PartitionedGraph &p_graph, tag::Sequential) {
  ...
  KASSERT(cut % 2 == 0u);
  cut /= 2;
  KASSERT((0 <= cut && cut <= std::numeric_limits<EdgeWeight>::max()),
          "edge cut overflows: " << cut, assert::always);
...
bool is_balanced(const PartitionedGraph &p_graph,
  ...
  return std::all_of(p_graph.blocks().begin(), p_graph.blocks().end(),
...
bool is_feasible(const PartitionedGraph &p_graph, const BlockID input_k,
                 const double eps) {
