...
CLI::Option_group* create_partitioning_options(CLI::App* app, Context& ctx) {
    ...
    partitioning->add_option("-e,--epsilon", ctx.partition.epsilon, "Maximum allowed imbalance.")
...
CLI::Option_group* create_coarsening_options(CLI::App* app, Context& ctx) {
    ...
    coarsening
        ...
        ->capture_default_str();
    coarsening->add_option("--clustering-algorithm", ctx.coarsening.algorithm, "Clustering algorithm")
  ...
  - lp:   use parallel label propagation)")
        ->capture_default_str();
    coarsening->add_option("--cluster-weight-limit", ctx.coarsening.cluster_weight_limit)
        ->transform(CLI::CheckedTransformer(get_cluster_weight_limits()).description(""))
        ->description(R"(During coarsening, the weight of each cluster is limited by a maximum cluster weight.
...
Ways to compute the base values are:
  ...
  - zero:                 Set the cluster weight limit to 0 (c_max = 0), effectively disabling coarsening)")
    ...
    coarsening
        ->add_option(
            "--cluster-weight-multiplier", ctx.coarsening.cluster_weight_multiplier,
            "Multiplicator of the maximum cluster weight base value."
        )
        ->capture_default_str();
    coarsening
        ->add_option(
            "--coarsening-convergence-threshold", ctx.coarsening.convergence_threshold,
...
CLI::Option_group* create_lp_coarsening_options(CLI::App* app, Context& ctx) {
    ...
    lp->add_option(
          "--c-lp-num-iterations", ctx.coarsening.lp.num_iterations, "Maximum number of label propagation iterations"
    )
        ->capture_default_str();
    lp->add_option(
          "--c-lp-active-large-degree-threshold", ctx.coarsening.lp.large_degree_threshold,
          "Threshold for ignoring nodes with large degree"
    )
        ->capture_default_str();
    lp->add_option(
          "--c-lp-two-hop-threshold", ctx.coarsening.lp.two_hop_clustering_threshold,
          "Enable two-hop clustering if plain label propagation shrunk the graph by less than this factor"
    )
        ->capture_default_str();
    lp->add_option(
          "--c-lp-max-num-neighbors", ctx.coarsening.lp.max_num_neighbors, "Limit the neighborhood to this many nodes"
    )
        ->capture_default_str();
...
CLI::Option_group* create_initial_partitioning_options(CLI::App* app, Context& ctx) {
  ...
  - sync-parallel:  same as async-parallel, but process branches synchronously)")
    ...
    ip->add_option(
          "--i-rep-multiplier", ctx.initial_partitioning.repetition_multiplier,
    ...
    ip->add_option("--i-min-reps", ctx.initial_partitioning.min_num_repetitions)
        ->description(R"(Minimum number of adaptive bipartitioning repetitions per bipartitioning algorithm.
Algorithms might perform less repetitions if they are unlikely to find a competitive bipartition.)")
        ->capture_default_str();
    ip->add_option("--i-min-non-adaptive-reps", ctx.initial_partitioning.min_num_non_adaptive_repetitions)
        ->description(R"(Minimum number of adaptive bipartitioning repetitions per bipartitioning algorithm,
before an algorithm is excluded if it is unlikely to find a competitive bipartition.)")
        ->capture_default_str();
    ip->add_option(
          "--i-max-reps", ctx.initial_partitioning.max_num_repetitions,
          "(Deprecated, but must be larger than the minimum number of repetitions)"
    )
        ->capture_default_str();
    ip->add_option(
          "--i-num-seed-iterations", ctx.initial_partitioning.num_seed_iterations,
          "Number of attempts to find good seeds for BFS-based bipartitioning algorithms."
    )
        ->capture_default_str();
    ip->add_flag(
        "--i-use-adaptive-bipartitioner-selection", ctx.initial_partitioning.use_adaptive_bipartitioner_selection,
...
CLI::Option_group* create_initial_refinement_options(CLI::App* app, Context& ctx) {
    ...
    refinement->add_option("--i-r-algorithm", ctx.initial_partitioning.refinement.algorithm)
        ->transform(CLI::CheckedTransformer(get_2way_refinement_algorithms()).description(""))
        ->description(R"(Algorithm for 2-way refinement during initial bipartitioning:
...
CLI::Option_group* create_initial_fm_refinement_options(CLI::App* app, Context& ctx) {
    ...
    fm->add_option("--i-r-fm-stopping-rule", ctx.initial_partitioning.refinement.fm.stopping_rule)
  ...
  - adaptive: abort after it became unlikely to find a cut improvement)")
        ->capture_default_str();
    fm->add_option(
          "--i-r-fm-num-fruitless-moves", ctx.initial_partitioning.refinement.fm.num_fruitless_moves,
          "Number of fruitless moves before aborting a FM search (simple stopping rule)."
    )
        ->capture_default_str();
    fm->add_option(
          "--i-r-fm-alpha", ctx.initial_partitioning.refinement.fm.alpha, "Alpha factor (adaptive stopping rule)."
    )
        ->capture_default_str();
    fm->add_option(
          "--i-r-fm-num-iterations", ctx.initial_partitioning.refinement.fm.num_iterations, "Number of iterations."
    )
        ->capture_default_str();
    fm->add_option(
          "--i-r-fm-abortion-threshold", ctx.initial_partitioning.refinement.fm.improvement_abortion_threshold,
          "Stop FM iterations if the previous iteration improved the edge cut below this threshold."
    )
        ->capture_default_str();
...
CLI::Option_group* create_lp_refinement_options(CLI::App* app, Context& ctx) {
    ...
    lp->add_option(
          "--r-lp-num-iterations", ctx.refinement.lp.num_iterations, "Maximum number of label propagation iterations"
    )
        ->capture_default_str();
    lp->add_option(
          "--r-lp-active-large-degree-threshold", ctx.refinement.lp.large_degree_threshold,
          "Threshold for ignoring nodes with large degree"
    )
        ->capture_default_str();
    lp->add_option(
          "--r-lp-max-num-neighbors", ctx.refinement.lp.max_num_neighbors, "Limit the neighborhood to this many nodes"
    )
        ->capture_default_str();
...
CLI::Option_group* create_balancer_options(CLI::App* app, Context& ctx) {
  ...
  - greedy: use the balancing algorithm as described in the ESA'21 publication)")
    ...
    balancer->add_option("--r-b-timepoint", ctx.refinement.balancer.timepoint)
        ->transform(CLI::CheckedTransformer(get_balancing_timepoints()).description(""))
