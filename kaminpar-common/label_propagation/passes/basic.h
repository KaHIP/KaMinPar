/*******************************************************************************
 * Basic label propagation pass shared by rating-map implementations.
 *
 * @file:   basic.h
 ******************************************************************************/
#pragma once
...
public:
  ...
  using RatingMap = typename RatingMapAccessor::RatingMap;
  using TieBreakingBuffer = ScalableVector<ClusterID>;

  explicit BasicPass(Kernel &kernel) : _kernel(kernel) {}
  ...
  public:
    Local(Kernel &kernel, Stats &stats)
        : _kernel(kernel),
          ...
          _rating_map(RatingMapAccessor::get(kernel)),
