...
METIS_PartGraphKway(idx_t *nvtxs, idx_t *ncon, idx_t *xadj, idx_t *adjncy,
  ...
  static_assert(std::numeric_limits<uidx_t>::digits ==
                std::numeric_limits<NodeID>::digits);
  static_assert(std::numeric_limits<uidx_t>::digits ==
                std::numeric_limits<EdgeID>::digits);
  static_assert(std::numeric_limits<uidx_t>::digits ==
                std::numeric_limits<BlockID>::digits);
  static_assert(std::numeric_limits<idx_t>::digits ==
                std::numeric_limits<NodeWeight>::digits);
  static_assert(std::numeric_limits<idx_t>::digits ==
                std::numeric_limits<EdgeWeight>::digits);
  static_assert(std::numeric_limits<idx_t>::digits ==
                std::numeric_limits<BlockWeight>::digits);
  ...
  if (*ncon != 1) {
    std::cout << "ignoring additional balancing constraints (unsupported)"
              << std::endl;
  }
  ...
  if (options != nullptr) {
    ...
    if (options[METIS_OPTION_NITER] > 0) {
      partitioner.set_option("--r-lp-num-iters",
                             std::to_string(options[METIS_OPTION_NITER]));
      partitioner.set_option("--i-r-fm-iterations",
                             std::to_string(options[METIS_OPTION_NITER]));
    }
    partitioner.set_option("--seed",
                           std::to_string(options[METIS_OPTION_SEED]));
  ...
  partitioner.set_option(
      "--threads", std::to_string(tbb::this_task_arena::max_concurrency()));
...
METIS_API(int)
METIS_MeshToDual(idx_t *, idx_t *, idx_t *, idx_t *, idx_t *, idx_t *, idx_t **,
                 idx_t **) {
  return METIS_ERROR;
}

METIS_API(int)
METIS_MeshToNodal(idx_t *, idx_t *, idx_t *, idx_t *, idx_t *, idx_t **,
                  idx_t **) {
  return METIS_ERROR;
}

METIS_API(int)
METIS_PartMeshNodal(idx_t *, idx_t *, idx_t *, idx_t *, idx_t *, idx_t *,
                    idx_t *, real_t *, idx_t *, idx_t *, idx_t *, idx_t *) {
  return METIS_ERROR;
}

METIS_API(int)
METIS_PartMeshDual(idx_t *, idx_t *, idx_t *, idx_t *, idx_t *, idx_t *,
                   idx_t *, idx_t *, real_t *, idx_t *, idx_t *, idx_t *,
                   idx_t *) {
  return METIS_ERROR;
}
...
METIS_API(int)
METIS_NodeNDP(idx_t, idx_t *, idx_t *, idx_t *, idx_t, idx_t *, idx_t *,
              idx_t *, idx_t *) {
  return METIS_ERROR;
}

METIS_API(int)
METIS_ComputeVertexSeparator(idx_t *, idx_t *, idx_t *, idx_t *, idx_t *,
                             idx_t *, idx_t *) {
  return METIS_ERROR;
}
