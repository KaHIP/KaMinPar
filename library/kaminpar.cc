...
PartitionerBuilder::PartitionerBuilder() {
  _pimpl = new PartitionerBuilderPrivate{};
}
PartitionerBuilder::~PartitionerBuilder() { delete _pimpl; }

PartitionerBuilder
PartitionerBuilder::from_graph_file(const std::string &filename) {
  PartitionerBuilder builder;
  shm::io::metis::read<true>(
      filename, builder._pimpl->nodes, builder._pimpl->edges,
      builder._pimpl->node_weights, builder._pimpl->edge_weights);
  builder._pimpl->n = builder._pimpl->nodes.size() - 1;
  return builder;
}

PartitionerBuilder PartitionerBuilder::from_adjacency_array(NodeID n,
                                                            EdgeID *nodes,
                                                            NodeID *edges) {
...
Partitioner PartitionerBuilder::create() {
  ...
  partitioner._pimpl->graph = Graph{
      std::move(_pimpl->nodes), std::move(_pimpl->edges),
      std::move(_pimpl->node_weights), std::move(_pimpl->edge_weights), false};
...
Partitioner PartitionerBuilder::rearrange_and_create() {
  ...
  // keep track of original total node weight to assign nodes before returning
  // the final partition
  if (_pimpl->node_weights.size() == 0) {
    partitioner._pimpl->original_total_node_weight = _pimpl->n;
  } else {
    partitioner._pimpl->original_total_node_weight =
        parallel::accumulate(_pimpl->node_weights, 0);
  }

  partitioner._pimpl->context = _pimpl->context;
  partitioner._pimpl->epsilon = _pimpl->context.partition.epsilon;
  partitioner._pimpl->permutations = graph::rearrange_graph(
      partitioner._pimpl->context.partition, _pimpl->nodes, _pimpl->edges,
      _pimpl->node_weights, _pimpl->edge_weights);
  partitioner._pimpl->graph = Graph{
      std::move(_pimpl->nodes), std::move(_pimpl->edges),
      std::move(_pimpl->node_weights), std::move(_pimpl->edge_weights), true};
  partitioner._pimpl->context.setup(partitioner._pimpl->graph);
  return partitioner;
}
...
namespace {
bool was_rearranged(PartitionerPrivate *pimpl) {
  return !pimpl->permutations.new_to_old.empty();
}

std::unique_ptr<BlockID[]> finalize_partition(Graph &graph,
                                              PartitionedGraph &p_graph,
                                              PartitionerPrivate *pimpl) {
  if (!was_rearranged(pimpl)) {
    std::unique_ptr<BlockID[]> new_partition =
        std::make_unique<BlockID[]>(graph.n());
    std::copy(p_graph.partition().begin(), p_graph.partition().end(),
              new_partition.get());
  ...
  const std::size_t restricted_node_weights_size =
      graph.raw_node_weights().size();
  ...
  for (NodeID u_prime = restricted_n; u_prime < graph.n(); ++u_prime) {
    ...
    while (b + 1 < k && block_weights[b] + graph.node_weight(u_prime) >
...
void adapt_epsilon_after_isolated_nodes_removal(
    const Graph &graph, PartitionContext &p_ctx,
    const NodeWeight original_total_node_weight) {
  const BlockID k = p_ctx.k;
  const double old_max_block_weight =
      (1 + p_ctx.epsilon) * std::ceil(1.0 * original_total_node_weight / k);
  const double new_epsilon =
      old_max_block_weight / std::ceil(1.0 * graph.total_node_weight() / k) - 1;
  p_ctx.epsilon = new_epsilon;
}
} // namespace

Partitioner &Partitioner::set_option(const std::string &name,
...
std::unique_ptr<BlockID[]> Partitioner::partition(BlockID k,
  ...
  if (was_rearranged(_pimpl)) {
    adapt_epsilon_after_isolated_nodes_removal(
        _pimpl->graph, _pimpl->context.partition,
        _pimpl->original_total_node_weight);
  }
  PartitionedGraph p_graph =
      partitioning::partition(_pimpl->graph, _pimpl->context);
...
void Partitioner::set_quiet(const bool quiet) { _quiet = quiet; }

void Partitioner::set_seed(const int seed) { _seed = seed; }
