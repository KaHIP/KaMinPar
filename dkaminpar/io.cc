...
DistributedGraph read_graph(const std::string &filename, const IOFormat format,
                            const IODistribution distribution, MPI_Comm comm) {
  const bool binary_file_extension =
      str::ends_with(filename, "bgf") || str::ends_with(filename, "bin");
  if (format == IOFormat::BINARY ||
      (format == IOFormat::AUTO && binary_file_extension)) {
...
DistributedGraph read_node_balanced(const std::string &filename,
  ...
  parse<false>(
      ...
      [&](const auto &format) {
        ...
        [[maybe_unused]] const auto global_m =
            static_cast<GlobalNodeID>(format.number_of_edges) * 2;
        DBG << "Loading graph with global_n=" << global_n
            << " and global_m=" << global_m;

        scalable_vector<GlobalNodeID> node_distribution(size + 1);
        for (PEID p = 0; p < size; ++p) {
          const auto [p_from, p_to] =
              math::compute_local_range<GlobalNodeID>(global_n, size, p);
      ...
      [&](const std::uint64_t &e_weight, const std::uint64_t &v) {
        if (current > from) {
          builder.create_edge(static_cast<EdgeWeight>(e_weight),
                              static_cast<GlobalNodeID>(v));
        }
      });
...
DistributedGraph read_edge_balanced(const std::string &filename,
  ...
  parse<false>(
      ...
      [&](const auto &format) {
        const auto global_n = static_cast<GlobalNodeID>(format.number_of_nodes);
        const auto global_m =
            static_cast<GlobalEdgeID>(format.number_of_edges) * 2;
        node_distribution.back() = global_n;
        edge_distribution.back() = global_m;
        const auto [pe_from, pe_to] =
            math::compute_local_range<GlobalEdgeID>(global_m, size, current_pe);
        to = pe_to;
      },
      [&](const std::uint64_t &u_weight) {
        if (current_edge >= to) {
          ...
          const auto [pe_from, pe_to] = math::compute_local_range<GlobalEdgeID>(
              global_m, size, current_pe);
          to = pe_to;
        }
  ...
  const auto local_n = static_cast<NodeID>(node_distribution[rank + 1] -
                                           node_distribution[rank]);
  ...
  for (std::size_t i = 0; i < global_edges.size(); ++i) {
    ...
    if (offset_n <= global_v && global_v < offset_n + local_n) { // owned node
      ...
      if (global_to_ghost.find(global_v) == global_to_ghost.end()) {
        ...
        auto it = std::upper_bound(node_distribution.begin() + 1,
                                   node_distribution.end(), global_v);
        const auto owner =
            static_cast<PEID>(std::distance(node_distribution.begin(), it) - 1);
...
void write(const std::string &filename, const DistributedGraph &graph,
           const bool write_node_weights, const bool write_edge_weights) {
...
DistributedGraph read_distributed_graph(std::ifstream &in,
                                        const GlobalNodeID from,
                                        const GlobalNodeID to, MPI_Comm comm) {
  ...
  IDType first_invalid_edge_index = 0;
  {
    ...
    tbb::parallel_for<std::size_t>(
        0, global_nodes.size(), [&](const std::size_t i) {
          nodes[i] = static_cast<EdgeID>((global_nodes[i] - first_edge_index) /
                                         sizeof(IDType));
        });
  }
  ...
  tbb::parallel_for<std::size_t>(
      0, global_edges.size(), [&](const std::size_t i) {
...
DistributedGraph read_node_balanced(const std::string &filename,
  ...
  const auto local_range =
      math::compute_local_range<GlobalNodeID>(global_n, size, rank);
  const auto &from = local_range.first;
  const auto &to = local_range.second;

  return read_distributed_graph(in, static_cast<GlobalNodeID>(from),
                                static_cast<GlobalNodeID>(to), comm);
}
...
IDType read_first_invalid_edge(std::ifstream &in, const IDType n,
                               const IDType u) {
  return read_first_edge(in, n, u + 1);
}

IDType compute_edge_balanced_from_node(std::ifstream &in, const IDType n,
                                       const IDType m, const int rank,
                                       const int size) {
  if (rank == 0) {
    return 0;
  } else if (rank == size) {
...
IDType compute_edge_balanced_to_node(std::ifstream &in, const IDType n,
                                     const IDType m, const int rank,
                                     const int size) {
...
DistributedGraph read_edge_balanced(const std::string &filename,
  ...
  const auto [size, rank] = mpi::get_comm_info(comm);
  const auto from =
      compute_edge_balanced_from_node(in, global_n, global_m, rank, size);
  const auto to =
      compute_edge_balanced_to_node(in, global_n, global_m, rank, size);
