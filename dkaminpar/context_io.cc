...
std::unordered_map<std::string, GlobalClusteringAlgorithm>
get_global_clustering_algorithms() {
  return {
...
std::unordered_map<std::string, LocalClusteringAlgorithm>
get_local_clustering_algorithms() {
  return {
...
std::unordered_map<std::string, ContractionAlgorithm>
get_contraction_algorithms() {
  return {
      {"legacy-no-migration", ContractionAlgorithm::LEGACY_NO_MIGRATION},
      {"legacy-minimal-migration",
       ContractionAlgorithm::LEGACY_MINIMAL_MIGRATION},
...
std::unordered_map<std::string, KWayRefinementAlgorithm>
...
void print(const PartitionContext &ctx, const bool root, std::ostream &out) {
  ...
  if (root) {
    out << "  Number of global nodes:    " << std::setw(width)
        << ctx.graph->global_n;
    ...
    out << "  Number of global edges:    " << std::setw(width)
        << ctx.graph->global_m;
    ...
    out << "Maximum block weight:         " << ctx.graph->max_block_weight(0)
        << " (" << ctx.graph->perfectly_balanced_block_weight(0) << " + "
        << 100 * ctx.epsilon << "%)\n";
  }
}
...
void print(
  ...
  if (ctx.max_global_clustering_levels > 0 &&
    ...
    out << "Coarsening mode:              local["
        << ctx.max_local_clustering_levels << "]+global["
        << ctx.max_global_clustering_levels << "]\n";
  } else if (ctx.max_global_clustering_levels > 0) {
    out << "Coarsening mode:              global["
        << ctx.max_global_clustering_levels << "]\n";
  } else if (ctx.max_local_clustering_levels > 0) {
    out << "Coarsening mode:              local["
        << ctx.max_local_clustering_levels << "]\n";
  } else {
  ...
  if (ctx.max_local_clustering_levels > 0) {
    ...
    out << "  High degree threshold:      "
        << ctx.local_lp.passive_high_degree_threshold << " (passive), "
  ...
  if (ctx.max_global_clustering_levels > 0) {
    ...
    if (ctx.contraction_algorithm == ContractionAlgorithm::DEFAULT &&
      ...
      out << "[rebalance if >" << std::setprecision(2)
          << 100.0 * (ctx.max_cnode_imbalance - 1.0) << "%";
    ...
    if (ctx.global_clustering_algorithm == GlobalClusteringAlgorithm::LP ||
      ...
      out << "  Number of iterations:       " << ctx.global_lp.num_iterations
          << "\n";
      out << "  High degree threshold:      "
          << ctx.global_lp.passive_high_degree_threshold << " (passive), "
      ...
      if (ctx.global_lp.fixed_num_chunks == 0) {
        out << "  Number of chunks:           "
            << ctx.global_lp.compute_num_chunks(parallel) << "[= max("
            << ctx.global_lp.min_num_chunks << ", "
            << ctx.global_lp.total_num_chunks << " / " << parallel.num_mpis
        ...
        out << "  Number of chunks:           "
            << ctx.global_lp.fixed_num_chunks << "\n";
      }
    ...
    if (ctx.global_clustering_algorithm == GlobalClusteringAlgorithm::HEM ||
      ...
      out << "  Small color blacklist:      "
          << 100 * ctx.hem.small_color_blacklist << "%"
...
void print(const RefinementContext &ctx, std::ostream &out) {
  ...
  out << "Refine initial partition:     "
      << (ctx.refine_coarsest_level ? "yes" : "no") << "\n";
  if (ctx.includes_algorithm(KWayRefinementAlgorithm::LP)) {
    ...
    //<< "\n";
    out << "  Use probabilistic moves:    "
        << (ctx.lp.ignore_probabilities ? "no" : "yes") << "\n";
  ...
  if (ctx.includes_algorithm(KWayRefinementAlgorithm::COLORED_LP)) {
    ...
    out << "  Commitment strategy:        "
        << ctx.colored_lp.move_execution_strategy << "\n";
    ...
    out << "  Commitment rounds:          "
        << ctx.colored_lp.num_move_execution_iterations << "\n";
    out << "  Track block weights:        "
        << (ctx.colored_lp.track_local_block_weights ? "yes" : "no") << "\n";
    out << "  Use active set:             "
        << (ctx.colored_lp.use_active_set ? "yes" : "no") << "\n";
    out << "  Small color blacklist:      "
        << 100 * ctx.colored_lp.small_color_blacklist << "%"
  ...
  if (ctx.includes_algorithm(KWayRefinementAlgorithm::GREEDY_BALANCER)) {
    out << "Greedy balancer:\n";
    out << "  Number of nodes per block:  "
        << ctx.greedy_balancer.num_nodes_per_block << "\n";
  }
}
} // namespace kaminpar::dist
