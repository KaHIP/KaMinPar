...
void DeeperPartitioningScheme::print_coarsening_level(const GlobalNodeWeight max_cluster_weight) const {
...
void DeeperPartitioningScheme::print_coarsening_converged() const {
    LOG << "==> Coarsening converged.";
    LOG;
}

void DeeperPartitioningScheme::print_coarsening_terminated(const GlobalNodeID desired_num_nodes) const {
    LOG << "==> Coarsening terminated with less than " << desired_num_nodes << " nodes.";
    LOG;
}

void DeeperPartitioningScheme::print_initial_partitioning_result(
...
DistributedPartitionedGraph DeeperPartitioningScheme::partition() {
...
Coarsener* DeeperPartitioningScheme::get_current_coarsener() {
    KASSERT(!_coarseners.empty());
    return &_coarseners.top();
}

const Coarsener* DeeperPartitioningScheme::get_current_coarsener() const {
    KASSERT(!_coarseners.empty());
    return &_coarseners.top();
}
} // namespace kaminpar::dist
