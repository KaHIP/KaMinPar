...
HEMClustering::HEMClustering(const Context &ctx)
    : _input_ctx(ctx), _ctx(ctx.coarsening.hem) {}
...
HEMClustering::compute_clustering(const DistributedGraph &graph,
  ...
  KASSERT(
      [&] {
        ...
        for (const NodeID u : _graph->nodes()) {
          ...
          KASSERT(_graph->contains_global_node(u_partner),
          ...
          if (_graph->is_owned_global_node(u_partner)) {
            ...
            KASSERT(u == local_partner || _matching[local_partner] == u_partner,
            ...
            *_graph,
            ...
            [&](const auto &r, const PEID pe) {
              for (const auto &[u, v] : r) {
                ...
                KASSERT(_matching[local_v] == v,
                        "invalid clustering structure for edge "
                            << u << " <-> " << v << " (local " << local_u
                            << " <-> " << local_v << "): expected " << v
                            << " to be the leader, but " << v
                            << " is in cluster " << _matching[local_v]);
              }
            });
...
void HEMClustering::compute_local_matching(
  ...
  _graph->pfor_nodes(seq_from, seq_to, [&](const NodeID seq_u) {
    ...
    if (best_weight > 0) {
      ...
      if (__atomic_compare_exchange_n(&_matching[best_neighbor], &unmatched,
                                      neighbor_global, true, __ATOMIC_SEQ_CST,
                                      __ATOMIC_SEQ_CST)) {
...
void HEMClustering::resolve_global_conflicts(const ColorID c) {
  ...
  auto all_requests =
      mpi::graph::sparse_alltoall_interface_to_ghost_custom_range_get<
          ...
          *_graph, seq_from, seq_to,
          ...
          [&](const NodeID u, const EdgeID e, const NodeID v,
              const PEID pe) -> MatchRequest {
  ...
  parallel::chunked_for(all_requests, [&](MatchRequest &req, const PEID pe) {
    ...
    } while (__atomic_compare_exchange_n(&_matching[req.mine], &current_partner,
                                         new_partner, true, __ATOMIC_SEQ_CST,
                                         __ATOMIC_SEQ_CST));
  });
  ...
  parallel::chunked_for(all_requests, [&](MatchRequest &req, const PEID pe) {
    ...
    req.theirs = static_cast<NodeID>(_graph->local_to_global_node(req.theirs) -
                                     _graph->offset_n(pe));
  });
  ...
  parallel::chunked_for(all_requests, [&](const MatchRequest &req) {
    if (req.mine != kInvalidNodeID) { // Due to the previous step, this should
      ...
      _matching[req.mine] = _graph->local_to_global_node(
          req.mine); // We become the leader of this cluster
    }
  });
