...
std::string create_basename(const Context &ctx, const int level) {
  return str::extract_basename(ctx.debug.graph_filename) + ".seed" +
         std::to_string(Random::seed) + ".k" + std::to_string(ctx.partition.k) +
         ".level" + std::to_string(level);
}
} // namespace

void save_partition(const DistributedPartitionedGraph &p_graph,
                    const Context &ctx, const int level) {
  ...
  if (mpi::get_comm_rank(p_graph.communicator()) == 0) {
    LOG_WARNING << "Writing partition of graph with " << p_graph.global_n()
                << " nodes and " << p_graph.global_m() << " edges to "
                << filename;
    LOG_WARNING << "  Cut: " << cut;
...
void save_graph(const DistributedGraph &graph, const Context &ctx,
                const int level) {
...
void save_partitioned_graph(const DistributedPartitionedGraph &p_graph,
                            const Context &ctx, const int level) {
