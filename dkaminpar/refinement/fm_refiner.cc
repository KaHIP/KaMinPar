...
public:
    struct Move {
        Move(const NodeID node, const EdgeWeight gain, const BlockID block) : node(node), gain(gain), block(block) {}
    ...
    ThreadLocalFMRefiner(
        const DistributedPartitionedGraph& global_p_graph, const shm::PartitionedGraph& p_graph,
        const FMRefinementContext& fm_ctx, const PartitionContext& p_ctx
    )
        : _global_p_graph(global_p_graph),
          ...
          _stopping_policy(_p_ctx.global_n()),
          _pq(_p_graph.n()),
          _marker(_p_graph.n()) {}
    ...
    std::vector<Move> refine(const NodeID start_node) {
        ...
        while (!_pq.empty() && !_stopping_policy.should_stop(_fm_ctx.alpha)) {
            ...
            const bool feasible =
                to != from
                && _global_p_graph.block_weight(to) + _block_weight_deltas[to] + weight <= _p_ctx.max_block_weight(to);
    ...
    std::pair<EdgeWeight, BlockID> find_best_target_block(const NodeID u) {
        ...
        auto action = [&](auto& map) {
            ...
            for (const auto [current_target_block, current_gain]: map.entries()) {
                ...
                BlockWeight block_weight_prime = _global_p_graph.block_weight(current_target_block) + u_weight;
                if (!initialization) {
...
void FMRefiner::refine(DistributedPartitionedGraph& p_graph) {
    ...
    for (std::size_t local_round = 0; local_round < _fm_ctx.num_iterations; ++local_round) {
        ...
        for (const auto& [node, group, weight, gain, from, to]: global_move_buffer) {
            ...
            if (_p_graph->contains_global_node(node)) {
                ...
                KASSERT(_p_graph->block(local_node) == from);
                _p_graph->set_block(local_node, to);
            }

            // Apply move to local graph
            KASSERT(reverse_node_mapping.find(node + 1) != reverse_node_mapping.end());
            const NodeID extracted_node = (*reverse_node_mapping.find(node + 1)).second;
            KASSERT(extraction_result.p_graph->block(extracted_node) == from);
