...
bool GreedyBalancer::refine() {
  ...
  IFSTATS(_stats.initial_feasible = metrics::is_feasible(_p_graph, _p_ctx));
  IFSTATS(_stats.initial_cut = metrics::edge_cut(_p_graph));
  IFSTATS(_stats.initial_imbalance = metrics::imbalance(_p_graph));
  IFSTATS(_stats.initial_num_imbalanced_blocks = metrics::num_imbalanced_blocks(_p_graph, _p_ctx));
  ...
  double previous_imbalance_distance =
      fast_balancing_enabled() ? metrics::imbalance_l1(_p_graph, _p_ctx) : 0.0;
  ...
  for (int round = 0; round < _ctx.refinement.greedy_balancer.max_num_rounds; round++) {
    ...
    if (fast_balancing_enabled()) {
      ...
      if ((previous_imbalance_distance - current_imbalance_distance) / previous_imbalance_distance <
        ...
        MPI_Bcast(
            cut_off_buckets.data(), num_overloaded_blocks, MPI_INT, 0, _p_graph.communicator()
        );
        STOP_TIMER();
        ...
        DBG0 << "Fast balancing round improved L1 imbalance from " << current_imbalance_distance
             << " to " << metrics::imbalance_l1(_p_graph, _p_ctx);
        ...
        TIMED_SCOPE("Reinit PQs") {
          DBG << "Reinit PQ, " << metrics::imbalance_l1(_p_graph, _p_ctx);
          init_pq();
          DBG << "Done";
          mpi::barrier(_p_graph.communicator());
        };
      ...
      previous_imbalance_distance = metrics::imbalance_l1(_p_graph, _p_ctx);
    }
  }

  IFSTATS(_stats.final_feasible = metrics::is_feasible(_p_graph, _p_ctx));
  IFSTATS(_stats.final_cut = metrics::edge_cut(_p_graph));
  IFSTATS(_stats.final_imbalance = metrics::imbalance(_p_graph));
  IFSTATS(_stats.final_num_imbalanced_blocks = metrics::num_imbalanced_blocks(_p_graph, _p_ctx));
