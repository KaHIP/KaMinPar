...
std::vector<NodeID> find_independent_border_set(const DistributedPartitionedGraph& p_graph, const int seed) {
    ...
    // Select nodes that have the lowest score in their neighborhood as independent set
    ...
    p_graph.pfor_nodes([&](const NodeID u) {
        ...
        const bool is_seed_node =
            std::all_of(p_graph.adjacent_nodes(u).begin(), p_graph.adjacent_nodes(u).end(), [&](const NodeID v) {
                // Compute score for ghost nodes lazy
                if (score[v] < 0) {
                    const auto v_score = compute_score(generator_ets.local(), p_graph.local_to_global_node(v), seed);
