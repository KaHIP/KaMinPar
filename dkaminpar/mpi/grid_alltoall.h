...
void sparse_alltoall_grid(SendBuffer &&data, const CountsBuffer &counts,
  ...
  // Exchange counts within payload
  {
    ...
    KASSERT(asserting_cast<int>(row_counts_send_counts.size()) >=
            col_comm_size);
    KASSERT(asserting_cast<int>(counts.size()) >=
            row_counts_send_displs.back());
    KASSERT(asserting_cast<int>(row_counts_recv_counts.size()) >=
            col_comm_size);
    KASSERT(asserting_cast<int>(row_counts_recv_displs.size()) >=
            col_comm_size);
    KASSERT(asserting_cast<int>(row_counts_send_displs.size()) >=
            col_comm_size);
  ...
  for (PEID col = 0; col < topo.num_full_cols(); ++col) {
    for (PEID row = 0; row < topo.virtual_col_size(col); ++row) {
      const PEID pe =
          topo.virtual_element(row, col); // row * col_comm_size + col;
