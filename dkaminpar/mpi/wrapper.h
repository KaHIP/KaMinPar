...
//
inline int barrier(MPI_Comm comm) { return MPI_Barrier(comm); }
...
template <typename T>
inline int reduce(const T *sendbuf, T *recvbuf, const int count, MPI_Op op,
                  const int root, MPI_Comm comm) {
  return MPI_Reduce(sendbuf, recvbuf, count, type::get<T>(), op, root, comm);
}

template <typename T>
inline int allreduce(const T *sendbuf, T *recvbuf, const int count, MPI_Op op,
                     MPI_Comm comm) {
  return MPI_Allreduce(sendbuf, recvbuf, count, type::get<T>(), op, comm);
}

template <typename Ts, typename Tr>
inline int scatter(const Ts *sendbuf, const int sendcount, Tr *recvbuf,
                   const int recvcount, const int root, MPI_Comm comm) {
  return MPI_Scatter(sendbuf, sendcount, type::get<Ts>(), recvbuf, recvcount,
                     type::get<Tr>(), root, comm);
}

template <typename Ts, typename Tr>
inline int gather(const Ts *sendbuf, const int sendcount, Tr *recvbuf,
                  const int recvcount, const int root, MPI_Comm comm) {
  return MPI_Gather(sendbuf, sendcount, type::get<Ts>(), recvbuf, recvcount,
                    type::get<Tr>(), root, comm);
}

template <typename Ts, typename Tr>
inline int allgather(const Ts *sendbuf, const int sendcount, Tr *recvbuf,
                     const int recvcount, MPI_Comm comm) {
  return MPI_Allgather(sendbuf, sendcount, type::get<Ts>(), recvbuf, recvcount,
                       type::get<Tr>(), comm);
}

template <typename Ts, typename Tr>
inline int alltoall(const Ts *sendbuf, const int sendcount, Tr *recvbuf,
                    const int recvcount, MPI_Comm comm) {
  return MPI_Alltoall(sendbuf, sendcount, type::get<Ts>(), recvbuf, recvcount,
                      type::get<Tr>(), comm);
}

template <typename Ts, typename Tr>
inline int alltoallv(const Ts *sendbuf, const int *sendcounts,
                     const int *sdispls, Tr *recvbuf, const int *recvcounts,
                     const int *rdispls, MPI_Comm comm) {
  return MPI_Alltoallv(sendbuf, sendcounts, sdispls, type::get<Ts>(), recvbuf,
                       recvcounts, rdispls, type::get<Tr>(), comm);
}

template <typename T>
inline int scan(const T *sendbuf, T *recvbuf, const int count, MPI_Op op,
                MPI_Comm comm) {
  return MPI_Scan(sendbuf, recvbuf, count, type::get<T>(), op, comm);
}

template <typename T>
inline int exscan(const T *sendbuf, T *recvbuf, const int count, MPI_Op op,
                  MPI_Comm comm) {
  return MPI_Exscan(sendbuf, recvbuf, count, type::get<T>(), op, comm);
}

template <typename T>
inline int reduce_scatter(const T *sendbuf, T *recvbuf, int *recvcounts,
                          MPI_Op op, MPI_Comm comm) {
  return MPI_Reduce_scatter(sendbuf, recvbuf, recvcounts, type::get<T>(), op,
                            comm);
}

template <typename Ts, typename Tr>
int gatherv(const Ts *sendbuf, const int sendcount, Tr *recvbuf,
            const int *recvcounts, const int *displs, const int root,
            MPI_Comm comm) {
  return MPI_Gatherv(sendbuf, sendcount, type::get<Ts>(), recvbuf, recvcounts,
                     displs, type::get<Tr>(), root, comm);
}

template <typename Ts, typename Tr>
int allgatherv(const Ts *sendbuf, const int sendcount, Tr *recvbuf,
               const int *recvcounts, const int *displs, MPI_Comm comm) {
  return MPI_Allgatherv(sendbuf, sendcount, type::get<Ts>(), recvbuf,
                        recvcounts, displs, type::get<Tr>(), comm);
}
...
template <typename T>
inline int send(const T *buf, const int count, const int dest, const int tag,
                MPI_Comm comm) {
  return MPI_Send(buf, count, type::get<T>(), dest, tag, comm);
}

template <typename T>
inline int isend(const T *buf, const int count, const int dest, const int tag,
                 MPI_Request *request, MPI_Comm comm) {
  return MPI_Isend(buf, count, type::get<T>(), dest, tag, comm, request);
}

template <typename T>
inline int recv(T *buf, int count, int source, int tag, MPI_Comm comm,
...
int send(const Container &buf, const int dest, const int tag, MPI_Comm comm) {
  return send(std::data(buf), asserting_cast<int>(std::size(buf)), dest, tag,
              comm);
}

template <typename Container,
          std::enable_if_t<!std::is_pointer_v<Container>, bool> = true>
int isend(const Container &buf, const int dest, const int tag,
          MPI_Request &request, MPI_Comm comm) {
  return isend(std::data(buf), asserting_cast<int>(std::size(buf)), dest, tag,
               &request, comm);
}

template <typename T, typename Buffer = NoinitVector<T>>
Buffer probe_recv(const int source, const int tag, MPI_Comm comm,
                  MPI_Status *status = MPI_STATUS_IGNORE) {
...
template <typename Container>
int waitall(Container &requests,
            MPI_Status *array_of_statuses = MPI_STATUS_IGNORE) {
  return MPI_Waitall(std::size(requests), std::data(requests),
                     array_of_statuses);
}
...
inline T reduce_single(const T &element, MPI_Op op, const int root,
                       MPI_Comm comm) {
  T ans = T{};
...
template <typename T, std::enable_if_t<!std::is_pointer_v<T>, bool> = true>
inline T reduce_single(const T &element, T &ans, MPI_Op op, const int root,
                       MPI_Comm comm) {
...
template <typename Container>
int gather(const typename Container::value_type &element, Container &ans,
           const int root, MPI_Comm comm) {
  KASSERT(mpi::get_comm_rank(comm) != root ||
...
template <typename Container>
inline int allgather(const typename Container::value_type &element,
                     Container &ans, MPI_Comm comm) {
  KASSERT(std::size(ans) >= static_cast<std::size_t>(mpi::get_comm_size(comm)),
          "", assert::light);
  return allgather(&element, 1, std::data(ans), 1, comm);
}

template <typename Rs, typename Rr, typename Rcounts, typename Displs>
int allgatherv(const Rs &sendbuf, Rr &recvbuf, const Rcounts &recvcounts,
               const Displs &displs, MPI_Comm comm) {
  static_assert(std::is_same_v<typename Rcounts::value_type, int>);
  static_assert(std::is_same_v<typename Displs::value_type, int>);
  return allgatherv(std::data(sendbuf), asserting_cast<int>(std::size(sendbuf)),
                    std::data(recvbuf), std::data(recvcounts),
...
inline int reduce(const R &sendbuf, R &recvbuf, MPI_Op op, const int root,
                  MPI_Comm comm) {
  KASSERT(mpi::get_comm_rank(comm) != root ||
              std::size(sendbuf) == std::size(recvbuf),
          "", assert::light);
  return reduce<typename R::value_type>(sendbuf.cdata(), std::data(recvbuf),
                                        asserting_cast<int>(std::size(sendbuf)),
                                        op, root, comm);
}

template <typename R, template <typename> typename Container = NoinitVector,
...
inline auto reduce(const R &sendbuf, MPI_Op op, const int root, MPI_Comm comm) {
  ...
  reduce(sendbuf.cdata(), recvbuf.data(),
         asserting_cast<int>(std::size(sendbuf)), op, root, comm);
  return recvbuf;
}

template <typename Rs, typename Rr>
inline int gather(const Rs &sendbuf, Rr &recvbuf, const int root,
                  MPI_Comm comm) {
  ...
  return gather<rs_value_t, rr_value_t>(
      sendbuf.cdata(), asserting_cast<int>(std::size(sendbuf)),
      std::data(recvbuf), asserting_cast<int>(std::size(recvbuf)), root, comm);
}
...
inline Container<T> build_distribution_from_local_count(const T value,
                                                        MPI_Comm comm) {
...
template <typename T>
inline NoinitVector<int> build_counts_from_value(const T original_value,
                                                 MPI_Comm comm) {
...
template <typename T>
inline NoinitVector<int> build_displs_from_value(const T original_value,
                                                 MPI_Comm comm) {
  const int value = asserting_cast<int>(original_value);
...
template <typename T>
std::tuple<T, double, T, std::int64_t> gather_statistics(const T value,
                                                         MPI_Comm comm) {
