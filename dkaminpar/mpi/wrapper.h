...
template <typename T>
inline int reduce(const T* sendbuf, T* recvbuf, const int count, MPI_Op op, const int root, MPI_Comm comm) {
...
inline int
scatter(const Ts* sendbuf, const int sendcount, Tr* recvbuf, const int recvcount, const int root, MPI_Comm comm) {
    return MPI_Scatter(sendbuf, sendcount, type::get<Ts>(), recvbuf, recvcount, type::get<Tr>(), root, comm);
}

template <typename Ts, typename Tr>
inline int
gather(const Ts* sendbuf, const int sendcount, Tr* recvbuf, const int recvcount, const int root, MPI_Comm comm) {
    return MPI_Gather(sendbuf, sendcount, type::get<Ts>(), recvbuf, recvcount, type::get<Tr>(), root, comm);
}

template <typename Ts, typename Tr>
inline int allgather(const Ts* sendbuf, const int sendcount, Tr* recvbuf, const int recvcount, MPI_Comm comm) {
    return MPI_Allgather(sendbuf, sendcount, type::get<Ts>(), recvbuf, recvcount, type::get<Tr>(), comm);
}

template <typename Ts, typename Tr>
inline int alltoall(const Ts* sendbuf, const int sendcount, Tr* recvbuf, const int recvcount, MPI_Comm comm) {
...
template <typename T>
inline int isend(const T* buf, const int count, const int dest, const int tag, MPI_Request* request, MPI_Comm comm) {
    return MPI_Isend(buf, count, type::get<T>(), dest, tag, comm, request);
}

template <typename T>
inline int recv(T* buf, int count, int source, int tag, MPI_Comm comm, MPI_Status* status = MPI_STATUS_IGNORE) {
...
template <typename T>
inline T bcast(T ans, const int root, MPI_Comm comm) {
...
int allgatherv(const Rs& sendbuf, Rr& recvbuf, const Rcounts& recvcounts, const Displs& displs, MPI_Comm comm) {
...
template <typename T>
T scan(const T& sendbuf, MPI_Op op, MPI_Comm comm) {
    T recvbuf = T{};
...
template <typename T>
T exscan(const T& sendbuf, MPI_Op op, MPI_Comm comm) {
    T recvbuf = T{};
