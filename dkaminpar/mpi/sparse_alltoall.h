...
template <typename Message, typename Buffer, typename SendBuffers,
          typename Receiver>
void sparse_alltoall(tag::grid_tag, SendBuffers &&send_buffers,
                     Receiver &&receiver, MPI_Comm comm) {
  sparse_alltoall_grid<Message, Buffer>(std::forward<SendBuffers>(send_buffers),
                                        std::forward<Receiver>(receiver), comm);
}

template <typename Message, typename Buffer, typename SendBuffers,
          typename Receiver>
void sparse_alltoall(tag::sparse_isend_irecv_tag, SendBuffers &&send_buffers,
                     Receiver &&receiver, MPI_Comm comm) {
  sparse_alltoall_sparse<Message, Buffer>(
      std::forward<SendBuffers>(send_buffers), std::forward<Receiver>(receiver),
      comm);
}

template <typename Message, typename Buffer, typename SendBuffers,
          typename Receiver>
void sparse_alltoall(tag::alltoallv_tag, SendBuffers &&send_buffers,
                     Receiver &&receiver, MPI_Comm comm) {
...
template <typename Message, typename Buffer, typename SendBuffers,
          typename Receiver>
void sparse_alltoall(tag::complete_send_recv_tag, SendBuffers &&send_buffers,
                     Receiver &&receiver, MPI_Comm comm) {
...
bool use_sparse_grid_alltoall(const SendBuffers &send_buffers, MPI_Comm comm) {
  const std::size_t local_num_elements = parallel::accumulate(
      send_buffers.begin(), send_buffers.end(), 0,
...
template <typename Message, typename Buffer = NoinitVector<Message>,
          typename Receiver>
void sparse_alltoall(const std::vector<Buffer> &send_buffers,
                     Receiver &&receiver, MPI_Comm comm) {
  SCOPED_TIMER("Sparse Alltoall");
  if (internal::use_sparse_grid_alltoall(send_buffers, comm)) {
    sparse_alltoall<Message, Buffer>(tag::grid, send_buffers,
                                     std::forward<Receiver>(receiver), comm);
  } else {
    sparse_alltoall<Message, Buffer>(tag::default_sparse_alltoall, send_buffers,
                                     std::forward<Receiver>(receiver), comm);
  }
}
...
template <typename Message, typename Buffer = NoinitVector<Message>,
          typename Receiver>
void sparse_alltoall(std::vector<Buffer> &&send_buffers, Receiver &&receiver,
                     MPI_Comm comm) {
  SCOPED_TIMER("Sparse Alltoall");
  if (internal::use_sparse_grid_alltoall(send_buffers, comm)) {
    sparse_alltoall<Message, Buffer>(tag::grid, std::move(send_buffers),
                                     std::forward<Receiver>(receiver), comm);
  } else {
    sparse_alltoall<Message, Buffer>(tag::default_sparse_alltoall,
...
template <typename Message, typename Buffer = NoinitVector<Message>>
std::vector<Buffer> sparse_alltoall_get(std::vector<Buffer> &&send_buffers,
                                        MPI_Comm comm) {
...
template <typename Message, typename Buffer = NoinitVector<Message>>
std::vector<Buffer> sparse_alltoall_get(const std::vector<Buffer> &send_buffers,
                                        MPI_Comm comm) {
  std::vector<Buffer> recv_buffers(mpi::get_comm_size(comm));
  sparse_alltoall<Message, Buffer>(
      send_buffers,
...
template <typename T>
void sparse_alltoallv(T *sendbuf, const int *sendcounts, const int *sdispls,
                      T *recvbuf, const int *, const int *rdispls,
                      MPI_Comm comm) {
  const PEID size = mpi::get_comm_size(comm);
  std::vector<NoinitVector<T>> split_sendbuf;
  for (PEID pe = 0; pe < size; ++pe) {
    split_sendbuf.emplace_back(sendcounts[pe]);
    std::copy(sendbuf + sdispls[pe], sendbuf + sdispls[pe] + sendcounts[pe],
