...
namespace {
std::unordered_map<GlobalNodeID, NodeID> compute_mapping(const DistributedGraph &graph,
                                                         const scalable_vector<GlobalNodeID> &clustering) {
  const auto [size, rank] = mpi::get_comm_info(graph.communicator());
  const GlobalNodeID nodes_per_pe = graph.global_n() / size;
  ...
  for (NodeID u : graph.nodes()) {
    const PEID new_owner = static_cast<PEID>(clustering[u] / nodes_per_pe);
  ...
  // exchange messages
  std::vector<GlobalNodeID> local_labels;
  ...
  local_labels.shrink_to_fit();

  const GlobalNodeID local_num_labels = local_labels.size();
  const GlobalNodeID prefix_sum = mpi::scan(local_num_labels, MPI_SUM, graph.communicator());
  const GlobalNodeID c_n = mpi::bcast(prefix_sum, size - 1, graph.communicator());
  const GlobalNodeID num_smaller_ids = prefix_sum - local_num_labels;

  // build mapping
  std::unordered_map<GlobalNodeID, NodeID> label_mapping;

  GlobalNodeID cur_id = num_smaller_ids;
