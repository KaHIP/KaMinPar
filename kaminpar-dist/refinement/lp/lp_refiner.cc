...
private:
  ...
  bool perform_moves(
    ...
    if (use_distributed_weight_tracking()) {
      ...
      std::vector<std::vector<Message>> recvbufs =
          mpi::sparse_alltoall_get<Message, std::vector<Message>>(sendbufs, _graph->communicator());
      ...
      for (auto &recvbuf : recvbufs) {
        for (auto &[block, delta] : recvbuf) {
          // Delta now becomes the new block weight, thus no longer a delta
          delta = _p_graph->block_weight(block);
        }
      }

      std::vector<std::vector<Message>> respbufs =
          mpi::sparse_alltoall_get<Message, std::vector<Message>>(recvbufs, _graph->communicator());
      for (const auto &recvbuf : recvbufs) {
        for (const auto &[block, delta] : recvbuf) {
      ...
      mpi::inplace_sparse_allreduce(
          block_weight_deltas, _p_ctx->k, MPI_SUM, _graph->communicator()
      );
    }
    ...
    if (!feasible) {
      ...
      if (!use_distributed_weight_tracking()) {
        // (with the distributed strategy, we have already overwritten the block weights when we
...
private:
  ...
  void finalize_block_weights() {
    ...
    std::vector<Message> recvbuf(_p_graph->k());
