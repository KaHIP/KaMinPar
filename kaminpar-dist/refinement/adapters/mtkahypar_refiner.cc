...
bool MtKaHyParRefiner::refine() {
  ...
  NoinitVector<mt_kahypar_hypernode_weight_t> block_weights(_p_ctx.k);
  _p_graph.pfor_blocks([&](const BlockID b) {
    block_weights[b] = _p_ctx.graph->max_block_weight(b);
  });
  mt_kahypar_set_individual_target_block_weights(
      mt_kahypar_ctx, static_cast<mt_kahypar_partition_id_t>(_p_ctx.k), block_weights.data()
  );
  ...
  const mt_kahypar_hypernode_id_t num_vertices = _p_graph.n();
  const mt_kahypar_hyperedge_id_t num_edges = _p_graph.m() / 2; // Only need one direction
  ...
  _p_graph.pfor_nodes([&](const NodeID u) {
    vertex_weights[u] = static_cast<mt_kahypar_hypernode_weight_t>(_p_graph.node_weight(u));
    ...
    for (const auto [e, v] : _p_graph.neighbors(u)) {
      ...
      edge_weights[position] = static_cast<mt_kahypar_hypernode_weight_t>(_p_graph.edge_weight(e));
    }
  });
  ...
  DBG << "Partition metrics before Mt-KaHyPar refinement: cut=" << metrics::edge_cut(_p_graph)
      << " imbalance=" << metrics::imbalance(_p_graph);

  NoinitVector<mt_kahypar_partition_id_t> partition(num_vertices);
  _p_graph.pfor_nodes([&](const NodeID u) { partition[u] = _p_graph.block(u); });
  ...
  _p_graph.pfor_nodes([&](const NodeID u) { _p_graph.set_block(u, improved_partition[u]); });

  DBG << "Partition metrics after Mt-KaHyPar refinement: cut=" << metrics::edge_cut(_p_graph)
      << " imbalance=" << metrics::imbalance(_p_graph);
