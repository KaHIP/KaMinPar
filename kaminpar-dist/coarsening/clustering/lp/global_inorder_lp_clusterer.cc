 ...
 *
 * @file:   global_inorder_lp_clusterer.cc
...
class GlobalInOrderLPClusteringImpl final
    : public InOrderLabelPropagation<GlobalInOrderLPClusteringImpl, GlobalLPClusteringConfig>,
      public NonatomicOwnedClusterVector<NodeID, GlobalNodeID> {
  SET_DEBUG(false);

  using Base = InOrderLabelPropagation<GlobalInOrderLPClusteringImpl, GlobalLPClusteringConfig>;
...
public:
  explicit GlobalInOrderLPClusteringImpl(const Context &ctx)
...
GlobalInOrderLPClusterer::GlobalInOrderLPClusterer(const Context &ctx)
    : _impl(std::make_unique<GlobalInOrderLPClusteringImpl>(ctx)) {}

GlobalInOrderLPClusterer::~GlobalInOrderLPClusterer() = default;

void GlobalInOrderLPClusterer::initialize(const DistributedGraph &graph) {
  _impl->initialize(graph);
}

GlobalInOrderLPClusterer::ClusterArray &GlobalInOrderLPClusterer::cluster(
