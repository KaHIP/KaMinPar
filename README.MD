...
```shell
# For AMD64
wget https://github.com/KaHIP/KaMinPar/releases/download/v3.7.2/kaminpar_3.7.2-1_amd64.deb
sudo apt install ./kaminpar_3.7.2-1_amd64.deb

# For ARM64
wget https://github.com/KaHIP/KaMinPar/releases/download/v3.7.2/kaminpar_3.7.2-1_arm64.deb
sudo apt install ./kaminpar_3.7.2-1_arm64.deb
```
...
```shell
# For AMD64
sudo dnf install https://github.com/KaHIP/KaMinPar/releases/download/v3.7.2/kaminpar-3.7.2-1.x86_64.rpm

# For ARM64
sudo dnf install https://github.com/KaHIP/KaMinPar/releases/download/v3.7.2/kaminpar-3.7.2-1.aarch64.rpm
```
...
./build/apps/KaMinPar <graph filename> <number of blocks> [-P default|terapart|eco|strong|largek] [-t <nproc>] [-e <eps, e.g., 0.03 for 3%>] [-o <output partition>]
...
* `-P eco`: better quality than `default` through additional FM refinement at the cost of increased runtime.
* `-P strong`: better quality than `eco` through additional flow refinement at the cost of increased runtime.
...
> [!CAUTION]
> Only one thread may call `KaMinPar::compute_partition()` at a time, even on different `KaMinPar` objects.
> To compute multiple partitions in parallel, disable the performance timers by passing `-DKAMINPAR_ENABLE_TIMERS=Off` to CMake.
...
@InProceedings{LinearTimeMultilevelGraphPartitioning,
  author       = {Lars Gottesb{\"{u}}ren and
                  Nikolai Maas and
                  Dominik Rosch and
                  Peter Sanders and
  ...
  publisher    = {Schloss Dagstuhl - Leibniz-Zentrum f{\"{u}}r Informatik},
  year         = {2025},
  url          = {https://doi.org/10.4230/LIPIcs.ESA.2025.32},
  doi          = {10.4230/LIPICS.ESA.2025.32},
  timestamp    = {Thu, 02 Oct 2025 15:21:33 +0200},
  biburl       = {https://dblp.org/rec/conf/esa/GottesburenMR0S25.bib},
  bibsource    = {dblp computer science bibliography, https://dblp.org}
}
```
