...
void write(const std::string &filename, const Container<UInt> &data,
  ...
  for (std::size_t i = 0; i < data.size(); ++i) {
    if (current_bit_pos + bits_per_entry > 64) {
      ...
      compact_data[current_entry_pos] |=
          (data[i] >> next_bits) & ((1 << prev_bits) - 1);
      compact_data[current_entry_pos + 1] |= (data[i] & ((1 << next_bits) - 1))
                                             << (64 - next_bits);
    } else {
      compact_data[current_entry_pos] |=
          (data[i] << (64 - current_bit_pos - bits_per_entry));
    }
