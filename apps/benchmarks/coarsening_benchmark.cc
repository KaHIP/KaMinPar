...
int main(int argc, char* argv[]) {
    ...
    for (std::size_t i = 0; i < num_iterations; ++i) {
        ...
        if (flat) {
            ...
            LOG << "max_cluster_weight=" << max_cluster_weight
                << " num_iterations=" << ctx.coarsening.lp.num_iterations;
            ...
            ClusteringCoarsener coarsener{
                std::make_unique<LabelPropagationClusteringAlgorithm>(graph.n(), ctx.coarsening), graph,
                ctx.coarsening};
            ...
            while (shrunk) {
                ...
                const auto   result =
                    coarsener.compute_coarse_graph(max_cluster_weight, 0); // might invalidate c_graph ptr
