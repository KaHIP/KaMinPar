...
int main(int argc, char* argv[]) {
    ...
    if (rank == 0) {
        MPI_Reduce(
            MPI_IN_PLACE, color_sizes.data(), asserting_cast<int>(num_colors), mpi::type::get<NodeID>(), MPI_SUM, 0,
            MPI_COMM_WORLD
        );
    } else {
        MPI_Reduce(
            color_sizes.data(), nullptr, asserting_cast<int>(num_colors), mpi::type::get<NodeID>(), MPI_SUM, 0,
            MPI_COMM_WORLD
        );
    }
