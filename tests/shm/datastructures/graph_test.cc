...
TEST(GraphTest, DegreeWorksForLeaves) {
  const Graph abstract_graph = make_graph({0, 1, 2}, {1, 0});
  const CSRGraph &graph = abstract_graph.csr_graph();
...
TEST(GraphTest, DegreeWorksForGraphWithIsolatedNodes) {
  const Graph abstract_graph = make_graph({0, 1, 1, 1, 2}, {3, 0});
  const CSRGraph &graph = abstract_graph.csr_graph();
...
TEST_F(AWeightedGridGraph, InitialBlockWeightsAreCorrect) {
  PartitionedGraph p_graph = make_p_graph(abstract_graph, 4, {0, 0, 1, 1, 2, 2, 3, 3});
...
TEST(GraphTest, IfBucketsAreDisabledNodesAreInFirstBucket) {
  const Graph abstract_graph = make_grid_graph(4, 4);
  const CSRGraph &graph = abstract_graph.csr_graph();
...
TEST(GraphTest, PutsIsolatedNodesInCorrectBucket) {
  const Graph abstract_graph = make_empty_graph(10, true);
  const CSRGraph &graph = abstract_graph.csr_graph();
...
TEST(GraphTest, PutsMatchingInCorrectBucket) {
  const Graph abstract_graph = make_matching_graph(10, true);
  const CSRGraph &graph = abstract_graph.csr_graph();
...
TEST(GraphTest, PutsAxeInCorrectBuckets) {
  ...
  const Graph abstract_graph =
      make_graph({0, 0, 1, 3, 5, 8, 12}, {5, 4, 5, 4, 5, 2, 3, 5, 1, 2, 3, 4}, true);
  const CSRGraph &graph = abstract_graph.csr_graph();
