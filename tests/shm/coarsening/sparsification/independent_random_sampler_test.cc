...
TEST(IRTest, NormalizationFactorForSmallGraph1) {
  EXPECT_NEAR(
      ir_weight.exactNormalizationFactor(testgraph, testgraph.raw_edge_weights(), 6), 0.2, 1e-6
  );
}

TEST(IRTest, NormalizationFactorForSmallGraph2) {
  EXPECT_NEAR(
      ir_weight.exactNormalizationFactor(testgraph, testgraph.raw_edge_weights(), 4), 0.1, 1e-6
  );
}
