 ...
 *
 * @return Distributed graph with one node on each PE, nodes are connected in a circle.
 */
...
/*!
 * Creates a distributed graph with `num_nodes_per_pe` nodes per PE and zero edges.
 *
...
/*!
 * Creates a distributed graph with `2 * num_edges_per_pe` nodes on each PE, each pair
 * connected by an edge.
 *
 * @param num_edges_per_pe Number of edges on each PE, with distinct endpoints.
 * @return Distributed graph with `2 * num_edges_per_pe` nodes and `num_edges_per_pe` edges per PE.
 */
 ...
 *
 * @param num_nodes_per_pe Number of nodes per PE.
 * @return Distributed graph with a clique on `num_nodes_per_pe` nodes on each PE and `num_nodes_per_pe` global circles.
 */
