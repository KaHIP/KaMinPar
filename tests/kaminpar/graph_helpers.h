...
inline Graph create_graph(const std::vector<EdgeID> &nodes,
                          const std::vector<NodeID> &edges,
                          const bool sorted = false) {
  return Graph{static_array::create_from(nodes),
...
inline EdgeID find_edge_by_endpoints(const Graph &graph, const NodeID u,
                                     const NodeID v) {
...
inline Graph change_node_weight(Graph graph, const NodeID u,
                                const NodeWeight new_node_weight) {
  auto node_weights = graph.take_raw_node_weights();
  node_weights[u] = new_node_weight;
  return Graph{graph.take_raw_nodes(), graph.take_raw_edges(),
               std::move(node_weights), graph.take_raw_edge_weights(),
               graph.sorted()};
}

inline Graph change_edge_weight(Graph graph, const NodeID u, const NodeID v,
  ...
  return Graph{graph.take_raw_nodes(), graph.take_raw_edges(),
               graph.take_raw_node_weights(), std::move(edge_weights),
               graph.sorted()};
}

inline Graph assign_exponential_weights(Graph graph,
                                        const bool assign_node_weights,
                                        const bool assign_edge_weights) {
  KASSERT(!assign_node_weights ||
