...
inline Graph
create_graph(const std::vector<EdgeID>& nodes, const std::vector<NodeID>& edges, const bool sorted = false) {
...
inline PartitionedGraph create_p_graph(const Graph& graph, const BlockID k, const std::vector<BlockID>& partition) {
...
inline Graph change_node_weight(Graph graph, const NodeID u, const NodeWeight new_node_weight) {
    ...
    return Graph{
        graph.take_raw_nodes(), graph.take_raw_edges(), std::move(node_weights), graph.take_raw_edge_weights(),
        graph.sorted()};
}

inline Graph change_edge_weight(Graph graph, const NodeID u, const NodeID v, const EdgeWeight new_edge_weight) {
    ...
    return Graph{
        graph.take_raw_nodes(), graph.take_raw_edges(), graph.take_raw_node_weights(), std::move(edge_weights),
        graph.sorted()};
}

inline Graph assign_exponential_weights(Graph graph, const bool assign_node_weights, const bool assign_edge_weights) {
    KASSERT(
        !assign_node_weights
    ...
    return Graph{
        graph.take_raw_nodes(), graph.take_raw_edges(), std::move(node_weights), std::move(edge_weights),
        graph.sorted()};
}
} // namespace kaminpar::shm::testing
