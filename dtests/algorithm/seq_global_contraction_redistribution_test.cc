/*******************************************************************************
 * @file:   seq_global_contraction_redistribution_test.cc
 *
...
using namespace ::dkaminpar::graph;

using Clustering = scalable_vector<shm::parallel::IntegralAtomicWrapper<GlobalNodeID>>;
...
TEST_F(DistributedTriangles, TestFullContractionToEachPE) {
  ...
  for (PEID pe = 0; pe < size; ++pe) {
    ...
    auto [c_graph, mapping,
          m_ctx] = contract_clustering(graph, {cluster, cluster, cluster, cluster, cluster, cluster, cluster});
...
TEST_F(DistributedTriangles, ContractLocalTriangles) {
  ...
  const auto [c_graph, mapping, m_ctx] = contract_clustering(graph, clustering);
...
TEST_F(DistributedTriangles, TestTriangleContractionOnOnePE) {
  ...
  const auto [c_graph, c_mapping, m_ctx] = contract_clustering(graph, clustering);
